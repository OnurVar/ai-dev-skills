...
5. **Report to the user.** Tell them:
   ...
   - Next: `/ov-ios-plan <feature>` to start planning, `/ov-ios-review-deep` to review pending changes.
   - They can edit `.claude/ios-rules.md` for project-local overrides.
   - Re-running `/ov-ios-init` refreshes the rules file from upstream (will prompt before overwriting).
