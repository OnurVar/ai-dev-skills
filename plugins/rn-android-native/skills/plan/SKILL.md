...
The RN Android native rules (`rules/rn-android-native.md`) should be loaded via the project's CLAUDE.md. If `@.claude/rn-android-native-rules.md` is not imported, suggest the user run `/rn-android-native:init` first, then stop. If the project has pure Android code, `/android:init` should also be run — these rule sets are additive.
...
- **Codegen spec.** Required for TurboModule / Fabric. Define the TS/Flow spec before the Kotlin implementation; sibling `/rn-typescript:plan` owns the JS-side spec.
