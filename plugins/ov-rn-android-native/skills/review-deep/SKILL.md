...
`rules/rn-android-native.md` should be imported via the project's CLAUDE.md (run `/ov-rn-android-native-init` if not). If the repo also has pure Android code, the `android` plugin's rules should be imported via `/ov-android-init` as well — this plugin's rules are additive, not a replacement.
