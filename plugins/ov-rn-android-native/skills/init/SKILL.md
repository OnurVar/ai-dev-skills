...
5. **Report to the user.** Tell them:
   ...
   - Next: `/ov-rn-android-native-plan <module>` to plan a native module or Fabric view, `/ov-rn-android-native-review-deep` to review pending changes.
   - They can edit `.claude/rn-android-native-rules.md` for project-local overrides.
   - Re-running `/ov-rn-android-native-init` refreshes the rules file from upstream (will prompt before overwriting).
   - This plugin's rules are **additive** to the pure-Android plugin. If the project also has plain Android code, also run `/ov-android-init` so Compose / pure-app rules are loaded too. Also run `/ov-rn-typescript-init` for the JS-side rules.
