...
The RN TypeScript rules (`rules/rn-typescript.md` from this plugin) should be loaded via the project's CLAUDE.md. If `@.claude/rn-typescript-rules.md` is not imported in CLAUDE.md, suggest the user run `/rn-typescript:init` first, then stop.
...
- Does this feature require a native module? If yes, flag it for `/rn-ios-native:plan` and `/rn-android-native:plan` follow-ups.
