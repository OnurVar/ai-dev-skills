...
5. **Report to the user.** Tell them:
   ...
   - Next: `/ov-rn-ios-native-plan <module>` to plan a native module or Fabric component, `/ov-rn-ios-native-review-deep` to review pending changes.
   - They can edit `.claude/rn-ios-native-rules.md` for project-local overrides.
   - Re-running `/ov-rn-ios-native-init` refreshes the rules file from upstream (will prompt before overwriting).
   - This plugin's rules are **additive** to the pure-iOS plugin. If the project also has plain iOS code, also run `/ov-ios-init` so SwiftUI / pure-app rules are loaded too. Also run `/ov-rn-typescript-init` for the JS-side rules.
