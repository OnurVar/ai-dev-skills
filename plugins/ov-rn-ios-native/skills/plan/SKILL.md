...
The RN iOS native rules (`rules/rn-ios-native.md`) should be loaded via the project's CLAUDE.md. If `@.claude/rn-ios-native-rules.md` is not imported, suggest the user run `/ov-rn-ios-native-init` first, then stop. If the project has pure iOS code, `/ov-ios-init` should also be run — these rule sets are additive.
...
- **Codegen spec.** Required for TurboModule / Fabric. Define the TS/Flow spec before the Swift/ObjC implementation; sibling `/ov-rn-typescript-plan` owns the JS-side spec.
