...
`rules/rn-ios-native.md` should be imported via the project's CLAUDE.md (run `/rn-ios-native:init` if not). If the repo also has pure iOS code, the `ios` plugin's rules should be imported via `/ios:init` as well — this plugin's rules are additive, not a replacement.
